    ...
    def create_video_recording(self, title, path, start_record, created_at):
        ...
        try:
            if cnx:
                cursor = cnx.cursor()
                cursor.execute(f"""
                    INSERT INTO camera_home_camerabentrancesavevideos (title, video, start_recording, created_at) VALUES (%s, %s, %s, %s);
    ...
    def delete_video_recording(self, name):
        ...
        try:
            if cnx:
                cursor = cnx.cursor()
                cursor.execute(f"""
                    DELETE FROM camera_home_camerabentrancesavevideos WHERE title = %s;
                """, (name,))
                cnx.commit()
        except mysql.connector.Error as e:
